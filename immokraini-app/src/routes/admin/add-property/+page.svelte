...
<div class="max-w-3xl mx-auto bg-white p-6 md:p-8 rounded-lg shadow-md">
    ...
    {#if submissionSuccess}
         <div class="mb-4 p-3 bg-green-100 text-green-700 rounded-md text-sm border border-green-300 flex items-center gap-2">
            ...
            <span>Property "{addedTitle}" added successfully!</span>
        </div>
    {/if}
    ...
    <form method="POST" use:enhance={handleSubmit} enctype="multipart/form-data" class="space-y-4" on:input={clearMessages}>
        ...
        <div> <label for="title" class="label">Title *</label> <input type="text" id="title" name="title" required class="form-input"> </div>
        <div> <label for="slug" class="label">Slug *</label> <input type="text" id="slug" name="slug" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" title="lowercase-letters-numbers-hyphens" class="form-input"> </div>
        <div> <label for="address" class="label">Address *</label> <input type="text" id="address" name="address" required class="form-input"> </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div> <label for="price" class="label">Price (TND) *</label> <input type="number" id="price" name="price" required min="0" class="form-input"> </div>
            <div> <label for="beds" class="label">Beds</label> <input type="number" id="beds" name="beds" min="0" class="form-input"> </div>
            <div> <label for="baths" class="label">Baths</label> <input type="number" id="baths" name="baths" min="0" class="form-input"> </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div> <label for="area" class="label">Area (sqm)</label> <input type="number" id="area" name="area" min="0" class="form-input"> </div>
            <div> <label for="propertyType" class="label">Property Type</label> <input type="text" id="propertyType" name="propertyType" placeholder="e.g., Villa" class="form-input"> </div>
            <div> <label for="yearBuilt" class="label">Year Built</label> <input type="number" id="yearBuilt" name="yearBuilt" min="1800" max={new Date().getFullYear() + 5} class="form-input"> </div>
        </div>
        <div> <label for="description" class="label">Description</label> <textarea id="description" name="description" rows="5" class="form-textarea"></textarea> </div>
        <div> <label for="features" class="label">Features (Comma-separated)</label> <input type="text" id="features" name="features" placeholder="e.g., Pool, Garden" class="form-input"> </div>
        <div> <label for="videoUrl" class="label">Video URL (Embed Link)</label> <input type="url" id="videoUrl" name="videoUrl" placeholder="e.g., https://youtube.com/embed/..." class="form-input"> </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div> <label for="latitude" class="label">Latitude</label> <input type="number" step="any" id="latitude" name="latitude" class="form-input"> </div>
            <div> <label for="longitude" class="label">Longitude</label> <input type="number" step="any" id="longitude" name="longitude" class="form-input"> </div>
        </div>
        <div> <label for="agentId" class="label">Agent ID (Temporary)</label> <input type="text" id="agentId" name="agentId" placeholder="Enter Agent CUID" class="form-input"> </div>
        


        <div>
            <label for="imageUrl" class="label">Main Image (Upload)</label>
            <input type="file" id="imageUrl" name="imageUrl" accept="image/*" class="file-input">
        </div>

         <div>
            <label for="galleryImages" class="label">Gallery Images (Upload Multiple)</label>
